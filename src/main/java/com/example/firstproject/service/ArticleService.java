...
public class ArticleService {
    ...
    public Article create(ArticleForm dto) {
        Article article = dto.toEntity(); // dto -> 엔티티 변환 후 article에 저장
        if (article.getId() != null) { // article 객체에 id가 존재한다면(null이 아니라면)
    ...
    public List<Article> createdArticles(List<ArticleForm> dtos) {
        ...
        List<Article> articleList = dtos.stream() // 1-1 dtos를 스트림화, 1-4 최종 결과를 articleList에 저장
                .map(dto -> dto.toEntity()) // 1-2 map()으로 dto가 하나하나 올 때마다 dto.toEntity()를 수행해 매핑
                .collect(Collectors.toList()); //1-3 이렇게 매힝한 것을 리스트로 묶기
        // 2. 엔티티 묶음을 DB에 저장
        articleList.stream()
